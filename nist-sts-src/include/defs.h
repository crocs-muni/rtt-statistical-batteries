...
struct results {
  struct frequency_str {
    double sum, sum_n, p_value;
  } frequency;
  struct blockfrequency_str {
    double chi_squared, p_value;
  } blockfrequency;
  struct runs_str {
    double pi, V, erfc_arg, p_value;
  } runs;
  struct longestrunofones_str {
    int N, M;
    double chi2, p_value;
    unsigned int nu[7];
  } longestrunofones;
  struct rank_str {
    double p_30, p_31, p_32, F_30, F_31, F_32, N, chi_squared, p_value;
  } rank;
  struct serial_str {
    double p_value1, p_value2, psim0, psim1, psim2, del1, del2;
  } serial;
  struct nonoverlapping_str {
    unsigned int templates;
    unsigned int *W;
    double *chi2;
    double *p_value;
  } nonoverlapping;
  struct overlapping_str {
    unsigned int nu[6];
    double chi2, p_value;
  } overlapping;
  struct universal_str {
    double p_value, sum, phi;
  } universal;
  struct approximate_entropy_str {
    double ApEn[2], chi_squared, p_value;
    unsigned int *P, pp;
  } approximate_entropy;
  struct cusum_str {
    int z, zrev;
    double sum1A, sum2A, sum1B, sum2B, p_valueA, p_valueB;
  } cusum;
  struct random_excursion_str {
    double p_value[8], sum[8];
    int valid, x[8], J[8];
  } random_excursion;
  struct random_excursion_variant_str {
    double p_value[18];
    int valid, x[18], count[18];
  } random_excursion_variant;
  struct linear_complexity_str {
    double p_value, chi2;
    int nu[7];
  } linear_complexity;
  struct dft_str {
    double p_value, percentile, N_l, N_o, d;
  } dft;
};
#endif
