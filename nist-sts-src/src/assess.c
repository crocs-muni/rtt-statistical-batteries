...
int main(int argc, char *argv[]) {
  ...
  if (cmdFlags.argCounter != argc - 1 || tp.n == 0) {
    printf("assess v5: incorect arguments.\n\n");
  ...
  if (cmdFlags.fileGen == 0)
    option = generatorOptions(&streamFile);

  if (cmdFlags.tests == 0)
    chooseTests();
  if (cmdFlags.fixArguments == 0)
    fixParameters();
...
void postProcessResults(int option) {
  ...
  for (i = 1; i <= NUMOFTESTS; i++) { // FOR EACH TEST
    if (testVector[i]) {
      ...
      if (((i == TEST_CUSUM) && testVector[TEST_CUSUM]) || ((i == TEST_NONPERIODIC) && testVector[TEST_NONPERIODIC]) ||
...
void postProcessResults() {
  ...
  if (testVector[1])
    process_pvals_single_test(pvals.frequency_pvals, TEST_FREQUENCY);
  if (testVector[2])
    process_pvals_single_test(pvals.blockfrequency_pvals, TEST_BLOCK_FREQUENCY);
  if (testVector[3]) {
    process_pvals_single_test(pvals.cusum_pvals[0], TEST_CUSUM);
    process_pvals_single_test(pvals.cusum_pvals[1], TEST_CUSUM);
  }
  if (testVector[4])
    process_pvals_single_test(pvals.runs_pvals, TEST_RUNS);
  if (testVector[5])
    process_pvals_single_test(pvals.longestrunofones_pvals, TEST_LONGEST_RUN);
  if (testVector[6])
    process_pvals_single_test(pvals.rank_pvals, TEST_RANK);
  if (testVector[7])
    process_pvals_single_test(pvals.dft_pvals, TEST_FFT);
  if (testVector[8])
    for (i = 0; i < pvals.num_Nonoverlap_pvals; i++)
      process_pvals_single_test(pvals.nonoverlapping_pvals[i], TEST_NONPERIODIC);
  if (testVector[9])
    process_pvals_single_test(pvals.overlapping_pvals, TEST_OVERLAPPING);
  if (testVector[10])
    process_pvals_single_test(pvals.universal_pvals, TEST_UNIVERSAL);
  if (testVector[11])
    process_pvals_single_test(pvals.approximate_entropy_pvals, TEST_APEN);
  if (testVector[12])
    for (i = 0; i < 8; i++)
  ...
  if (testVector[15])
    process_pvals_single_test(pvals.linear_complexity_pvals, TEST_LINEARCOMPLEXITY);
...
void partitionResultFile(int numOfFiles, int numOfSequences, int option, int testNameID) {
  ...
  switch (testNameID) {
  case TEST_CUSUM:
    arr = pvals.cusum_pvals;
    break;
  case TEST_NONPERIODIC:
    arr = pvals.nonoverlapping_pvals;
    break;
  case TEST_RND_EXCURSION:
    arr = pvals.random_excursion_pvals;
    break;
  case TEST_RND_EXCURSION_VAR:
    arr = pvals.random_excursion_variant_pvals;
    break;
  case TEST_SERIAL:
    arr = pvals.serial_pvals;
    break;
  }
