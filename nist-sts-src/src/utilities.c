...
void fixParameters() {
  ...
  if ((testVector[TEST_BLOCK_FREQUENCY] != 1) && (testVector[TEST_NONPERIODIC] != 1) &&
      (testVector[TEST_OVERLAPPING] != 1) && (testVector[TEST_APEN] != 1) && (testVector[TEST_SERIAL] != 1) &&
...
void openOutputStreams(int option) {
  ...
  if (cmdFlags.bitStreams == 0) {
    ...
    numOfBitStreams = tp.numOfBitStreams;

#ifdef KS

  if (testVector[1])
    pvals.frequency_pvals = malloc(sizeof(double) * numOfBitStreams);
  if (testVector[2])
    pvals.blockfrequency_pvals = malloc(sizeof(double) * numOfBitStreams);
  if (testVector[3])
    pvals.cusum_pvals[0] = malloc(sizeof(double) * numOfBitStreams);
  if (testVector[3])
    pvals.cusum_pvals[1] = malloc(sizeof(double) * numOfBitStreams);
  if (testVector[4])
    pvals.runs_pvals = malloc(sizeof(double) * numOfBitStreams);
  if (testVector[5])
    pvals.longestrunofones_pvals = malloc(sizeof(double) * numOfBitStreams);
  if (testVector[6])
    pvals.rank_pvals = malloc(sizeof(double) * numOfBitStreams);
  if (testVector[7])
    pvals.dft_pvals = malloc(sizeof(double) * numOfBitStreams);
  if (testVector[8])
    for (i = 0; i < MAXNUMOFTEMPLATES; i++)
      pvals.nonoverlapping_pvals[i] = (double *)malloc(sizeof(double) * numOfBitStreams);
  if (testVector[9])
    pvals.overlapping_pvals = malloc(sizeof(double) * numOfBitStreams);
  if (testVector[10])
    pvals.universal_pvals = malloc(sizeof(double) * numOfBitStreams);
  if (testVector[11])
    pvals.approximate_entropy_pvals = malloc(sizeof(double) * numOfBitStreams);
  if (testVector[12])
    for (i = 0; i < 8; i++)
      pvals.random_excursion_pvals[i] = malloc(sizeof(double) * numOfBitStreams);
  if (testVector[13])
    for (i = 0; i < 18; i++)
      pvals.random_excursion_variant_pvals[i] = malloc(sizeof(double) * numOfBitStreams);
  if (testVector[14])
    pvals.serial_pvals[0] = malloc(sizeof(double) * numOfBitStreams);
  if (testVector[14])
    pvals.serial_pvals[1] = malloc(sizeof(double) * numOfBitStreams);
  if (testVector[15])
    pvals.linear_complexity_pvals = malloc(sizeof(double) * numOfBitStreams);

  pvals.seq_counter = 0;
  //***************************
  if (testVector[8])
    pvals.num_Nonoverlap_pvals = numOfTemplates[tp.nonOverlappingTemplateBlockLength];
...
void nist_test_suite() {
  ...
  if ((testVector[0] == 1) || (testVector[TEST_BLOCK_FREQUENCY] == 1)) {
    if (tp.fast)
      BlockFrequency_v2(tp.blockFrequencyBlockLength, tp.n);
    else
      BlockFrequency_v1(tp.blockFrequencyBlockLength, tp.n);
  }

  if ((testVector[0] == 1) || (testVector[TEST_CUSUM] == 1)) {
    if (tp.fast)
      CumulativeSums_v2(tp.n);
    else
      CumulativeSums_v1(tp.n);
  }
  ...
  if ((testVector[0] == 1) || (testVector[TEST_LONGEST_RUN] == 1)) {
    if (tp.fast)
      LongestRunOfOnes_v2(tp.n);
    else
      LongestRunOfOnes_v1(tp.n);
  }
  ...
  if ((testVector[0] == 1) || (testVector[TEST_FFT] == 1)) {
    if (tp.fast)
      DiscreteFourierTransform_v2(tp.n);
    else
      DiscreteFourierTransform_v1(tp.n);
  }

  if ((testVector[0] == 1) || (testVector[TEST_NONPERIODIC] == 1)) {
    if (tp.fast)
      NonOverlappingTemplateMatchings_v2(tp.nonOverlappingTemplateBlockLength, tp.n);
    else
      NonOverlappingTemplateMatchings_v1(tp.nonOverlappingTemplateBlockLength, tp.n);
  }

  if ((testVector[0] == 1) || (testVector[TEST_OVERLAPPING] == 1)) {
    if (tp.fast)
      OverlappingTemplateMatchings_v2(tp.overlappingTemplateBlockLength, tp.n);
    else
      OverlappingTemplateMatchings_v1(tp.overlappingTemplateBlockLength, tp.n);
  }
  ...
  if ((testVector[0] == 1) || (testVector[TEST_APEN] == 1)) {
    if (tp.fast)
      ApproximateEntropy_v2(tp.approximateEntropyBlockLength, tp.n);
    else
      ApproximateEntropy_v1(tp.approximateEntropyBlockLength, tp.n);
  }

  if ((testVector[0] == 1) || (testVector[TEST_RND_EXCURSION] == 1)) {
    if (tp.fast)
      RandomExcursions_v2(tp.n);
    else
      RandomExcursions_v1(tp.n);
  }

  if ((testVector[0] == 1) || (testVector[TEST_RND_EXCURSION_VAR] == 1)) {
    if (tp.fast)
      RandomExcursionsVariant_v2(tp.n);
    else
      RandomExcursionsVariant_v1(tp.n);
  }

  if ((testVector[0] == 1) || (testVector[TEST_SERIAL] == 1)) {
    if (tp.fast)
      Serial_v2(tp.serialBlockLength, tp.n);
    else
      Serial_v1(tp.serialBlockLength, tp.n);
  }

  if ((testVector[0] == 1) || (testVector[TEST_LINEARCOMPLEXITY] == 1)) {
    if (tp.fast)
      LinearComplexity_v2(tp.linearComplexitySequenceLength, tp.n);
    else
      LinearComplexity_v1(tp.linearComplexitySequenceLength, tp.n);
  }
}
#ifdef KS
void freeMemory() {
